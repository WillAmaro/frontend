...
import {
  Box, Card, Typography, TextField, Divider, IconButton, Alert, Fade,
  Grow, Chip, Stack, InputAdornment, Avatar, Autocomplete, CircularProgress,
  LinearProgress, Tooltip, Tab, Tabs, Dialog, DialogTitle, DialogContent,
  DialogActions, Paper, ToggleButtonGroup, ToggleButton,
} from "@mui/material";
import {
  LocalShippingOutlined, CheckCircleOutline, SearchOutlined,
  CalendarTodayOutlined, PersonOutlined, PictureAsPdfOutlined,
  BadgeOutlined, QrCodeScannerOutlined, CheckCircle, StorageOutlined,
  RefreshOutlined, CloseOutlined, SendOutlined, InventoryOutlined,
  DeleteOutline, AddOutlined, RemoveOutlined, WarningAmberOutlined,
} from "@mui/icons-material";
import { TitleCard } from "@/src/components/base/TitleCard";
import ButtonBase from "@/src/components/base/ButtonBase";
import { API_URL } from "@/src/lib/config";
...
// Una unidad de equipo con su serie y datos técnicos
...
// Fila de inventario enriquecida con cantidad solicitada
interface InventoryRow extends HubInventoryItem {
  requestedQty: number;
  notes: string;
  equipmentUnits: EquipmentUnit[]; // solo EQUIPMENT
}
...
const P: Record<ProductTab, { label: string; icon: string; color: string; bg: string; border: string }> = {
  MATERIAL:  { label: "Materiales",   icon: "📦", color: "#ea580c", bg: "#fff7ed", border: "#fed7aa" },
  TOOL:      { label: "Herramientas", icon: "🔧", color: "#1d4ed8", bg: "#eff6ff", border: "#bfdbfe" },
  EQUIPMENT: { label: "Equipos",      icon: "⚙️", color: "#15803d", bg: "#f0fdf4", border: "#bbf7d0" },
  EPP:       { label: "EPP",          icon: "🦺", color: "#7c3aed", bg: "#faf5ff", border: "#ddd6fe" },
...
const ROLE_LABEL: Record<string, string> = {
  TECNICO: "Técnico", JEFE_CUADRILLA: "Jefe de Cuadrilla", SUPERVISOR: "Supervisor",
};
const ROLE_COLOR: Record<string, "primary" | "warning" | "success" | "default"> = {
  TECNICO: "primary", JEFE_CUADRILLA: "warning", SUPERVISOR: "success",
};

type Semaforo = "VERDE" | "ROJO" | "GRIS";
const getSem = (item: HubInventoryItem): Semaforo => {
  if (item.quantityAvailable <= 0) return "GRIS";
  if (item.quantityAvailable <= item.minimumStock) return "ROJO";
  return "VERDE";
};
const SEM: Record<Semaforo, { color: string; label: string }> = {
  VERDE: { color: "#16a34a", label: "OK"        },
  ROJO:  { color: "#dc2626", label: "Crítico"   },
  GRIS:  { color: "#94a3b8", label: "Sin stock" },
};
...
export default function DespachoCuadrillas() {
  ...
  // Inventory rows (la page activa del tab actual)
  ...
  // Map global: id → InventoryRow — persiste entre cambios de tab
  const [selected, setSelected] = useState<Map<number, InventoryRow>>(new Map());

  // Pistolado modal
  const [pistoleoRow,  setPistoleoRow]  = useState<InventoryRow | null>(null);
  const [pistoleoOpen, setPistoleoOpen] = useState(false);
  const [editUnit,     setEditUnit]     = useState<Partial<EquipmentUnit>>({});
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/workers`)
      .then(r => r.json())
      .then(j => {
        const data: Worker[] = Array.isArray(j) ? j : j.data ?? [];
        setWorkers(data.filter(w => w.status === "ACTIVE"));
      })
  ...
  // ── Debounce search ───────────────────────────────────────────────────────
  useEffect(() => {
    const t = setTimeout(() => { setDebSearch(invSearch); setInvPage(0); }, 350);
    return () => clearTimeout(t);
  }, [invSearch]);
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({
      ...
      page: String(invPage), size: "25", productType: activeTab,
      ...(debSearch.trim() ? { search: debSearch.trim() } : {}),
    });
    fetch(`${API_URL}/api/hub-inventory?${params}`)
      .then(r => r.json())
      .then(json => {
        const content: HubInventoryItem[] = json.data?.content ?? [];
        setInvTotal(json.data?.page?.totalElements ?? json.data?.totalElements ?? content.length);
        // Merge: si ya hay datos editados en selected, los preservamos
        setRows(content.map(item => {
          const prev = selected.get(item.id);
          return prev
            ? { ...item, requestedQty: prev.requestedQty, notes: prev.notes, equipmentUnits: prev.equipmentUnits }
            : { ...item, requestedQty: 0, notes: "", equipmentUnits: [] };
        }));
      })
      .catch(() => toast.error("Error al cargar inventario"))
      .finally(() => setLoadingInv(false));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTab, debSearch, invPage]);

  // ── Sync rows → selected ──────────────────────────────────────────────────
  // Cada vez que el usuario edita una fila la sincronizamos al Map global
  const syncSelected = (updatedRow: InventoryRow) => {
    setSelected(prev => {
      const next = new Map(prev);
      if (updatedRow.requestedQty > 0) {
  ...
  const setRowQty = (id: number, qty: number) => {
    setRows(prev => prev.map(r => {
      if (r.id !== id) return r;
      const clamped = Math.max(0, Math.min(r.quantityAvailable, qty));
      const units = r.productType === "EQUIPMENT"
        ? r.equipmentUnits.slice(0, clamped)
        : r.equipmentUnits;
      const updated = { ...r, requestedQty: clamped, equipmentUnits: units };
      syncSelected(updated);
      return updated;
    }));
  };

  const setRowNotes = (id: number, notes: string) => {
    setRows(prev => prev.map(r => {
      if (r.id !== id) return r;
  ...
  const totalUnits    = selectedItems.reduce((s, r) => s + r.requestedQty, 0);

  const equipmentWarnings = selectedItems
    .filter(r => r.productType === "EQUIPMENT" && r.equipmentUnits.length < r.requestedQty)
    .map(r => `${r.itemCode}: ${r.equipmentUnits.length}/${r.requestedQty} pistoleados`);

  // ── Pistolado ─────────────────────────────────────────────────────────────
  const openPistoleo = (row: InventoryRow) => {
    if (row.requestedQty === 0) { toast.warning("Ingresa la cantidad primero"); return; }
    // Refrescamos desde selected para tener la versión más reciente
    const latest = selected.get(row.id) ?? row;
    setPistoleoRow({ ...latest });
    setEditUnit({});
    setPistoleoOpen(true);
  };

  const applyUnitChange = (rowId: number, updater: (row: InventoryRow) => InventoryRow) => {
    // Actualiza rows, selected y pistoleoRow
    setRows(prev => prev.map(r => r.id === rowId ? updater(r) : r));
    setSelected(prev => {
      const next = new Map(prev);
      const ex = next.get(rowId);
      if (ex) next.set(rowId, updater(ex));
      return next;
    });
    setPistoleoRow(prev => prev && prev.id === rowId ? updater(prev) : prev);
  };

  const addUnit = () => {
    if (!pistoleoRow) return;
    if (!editUnit.serialNumber?.trim()) { toast.warning("Ingresa el número de serie"); return; }
    if (pistoleoRow.equipmentUnits.length >= pistoleoRow.requestedQty) {
      toast.info("Ya pistoleaste todos los equipos requeridos"); return;
    }
    const newUnit: EquipmentUnit = {
      unitId: uid(),
      serialNumber:  editUnit.serialNumber?.trim()  ?? "",
      macAddress:    editUnit.macAddress?.trim()     ?? "",
      mtaMacAddress: editUnit.mtaMacAddress?.trim()  ?? "",
      unitAddress:   editUnit.unitAddress?.trim()    ?? "",
    };
    applyUnitChange(pistoleoRow.id, r => ({ ...r, equipmentUnits: [...r.equipmentUnits, newUnit] }));
    setEditUnit({});
    toast.success(`Serie ${newUnit.serialNumber} registrada`);
  };

  const removeUnit = (rowId: number, unitId: string) =>
    applyUnitChange(rowId, r => ({ ...r, equipmentUnits: r.equipmentUnits.filter(u => u.unitId !== unitId) }));

  // ── Submit ────────────────────────────────────────────────────────────────
  const handleSubmit = async () => {
    if (!selectedWorker) { toast.warning("Selecciona un técnico"); return; }
    if (totalItems === 0) { toast.warning("Ingresa cantidad en al menos un ítem"); return; }
    ...
    try {
      const body = {
        ...
        workerDni:     selectedWorker.dni,
        workerName:    selectedWorker.name,
        items: selectedItems.flatMap(r => {
          if (r.productType === "EQUIPMENT") {
            return r.equipmentUnits.map(u => ({
              itemId:             r.itemId,
              inventoryId:        r.id,
              quantity:           1,
              notes:              r.notes,
              productType:        r.productType,
              supplySource:       r.supplySource ?? "CLARO",
              supplySourceEntityId: 1,
              serialNumber:       u.serialNumber  || null,
              macAddress:         u.macAddress    || null,
              mtaMacAddress:      u.mtaMacAddress || null,
              unitAddress:        u.unitAddress   || null,
            }));
          }
          return [{
            itemId:             r.itemId,
      ...
      const created = await res.json();
      const code = created?.data?.dispatchNumber ?? created?.data?.id ?? `DESP-${Date.now()}`;
      setDispatchCode(String(code));
      setShowSuccess(true);
      topRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  const handleReset = () => {
    setShowSuccess(false); setSelectedWorker(null); setWorkerInput("");
    setRows([]); setSelected(new Map()); setObservations("");
    setPriority("MEDIUM"); setActiveTab("MATERIAL"); setInvSearch("");
  };
  ...
  const handlePDF = async () => {
    ...
    doc.setFillColor(25,118,210); doc.rect(0,0,W,28,"F");
    doc.setFontSize(16); doc.setFont("helvetica","bold"); doc.setTextColor(255,255,255);
    doc.text("LPS GROUP S.A.C.", M, 11);
    doc.setFontSize(10); doc.setFont("helvetica","normal");
    doc.text("CARGO DE DESPACHO — LPS ERP", M, 18);
    doc.setFontSize(9); doc.setTextColor(200,230,255);
    doc.text(`Código: ${dispatchCode}`, W-M-55, 11);
    doc.text(`Fecha: ${dayjs().format("DD/MM/YYYY HH:mm")}`, W-M-55, 17);
    y = 36;

    doc.setFillColor(240,247,255); doc.roundedRect(M,y,CW,22,2,2,"F");
    doc.setDrawColor(25,118,210); doc.roundedRect(M,y,CW,22,2,2,"S");
    doc.setFontSize(9); doc.setFont("helvetica","bold"); doc.setTextColor(25,118,210);
    doc.text("TÉCNICO RECEPTOR", M+4, y+6);
    doc.setTextColor(50,50,50); doc.setFont("helvetica","normal");
    doc.text("Nombre: ", M+4, y+13); doc.setFont("helvetica","bold");
    ...
    doc.setFont("helvetica","normal");
    doc.text(`DNI: ${selectedWorker.dni}`, M+4, y+19);
    doc.text(`Rol: ${ROLE_LABEL[selectedWorker.role]??selectedWorker.role}`, M+60, y+19);
    y += 28;

    const drawSec = (title: string, rgb:[number,number,number], rows2: string[][]) => {
      if (!rows2.length) return;
      if (y>240) { doc.addPage(); y=15; }
      doc.setFillColor(...rgb); doc.roundedRect(M,y,CW,7,1,1,"F");
      doc.setTextColor(255,255,255); doc.setFont("helvetica","bold"); doc.setFontSize(9);
      doc.text(title, M+3, y+5); y += 10;
      const ws = [8,22,82,12,12,34];
      const hdrs = ["N°","CÓDIGO","DESCRIPCIÓN","CANT.","STOCK","NOTAS/SERIE"];
      doc.setFillColor(245,245,245); doc.rect(M,y,CW,6,"F");
      doc.setTextColor(80,80,80); doc.setFontSize(7.5); let xH = M+2;
      hdrs.forEach((h,i) => { doc.text(h,xH,y+4); xH+=ws[i]; }); y+=7;
      doc.setFont("helvetica","normal"); doc.setTextColor(40,40,40);
      rows2.forEach((row,ri) => {
        if (y>270) { doc.addPage(); y=15; }
        if (ri%2===0) { doc.setFillColor(252,252,252); doc.rect(M,y,CW,6,"F"); }
        let xR = M+2;
        row.forEach((cell,ci) => { doc.text(doc.splitTextToSize(cell,ws[ci]-2)[0]??cell,xR,y+4); xR+=ws[ci]; });
        y+=6; doc.setDrawColor(235,235,235); doc.line(M,y,W-M,y);
      }); y+=5;
    };

    (["MATERIAL","TOOL","EPP"] as ProductTab[]).forEach(type => {
      const items = selectedItems.filter(r => r.productType===type);
      if (!items.length) return;
      const cfg = P[type];
      const rgb = cfg.color.replace("#","").match(/.{2}/g)!.map(h=>parseInt(h,16)) as [number,number,number];
      drawSec(`${cfg.icon} ${cfg.label.toUpperCase()}`, rgb,
        items.map((r,i) => [String(i+1), r.itemCode, (r.description||r.itemName).substring(0,36),
          String(r.requestedQty), String(r.quantityAvailable), r.notes||"—"]));
    });

    const eqItems = selectedItems.filter(r => r.productType==="EQUIPMENT");
    if (eqItems.length) {
      drawSec("⚙️ EQUIPOS", [21,128,61],
        eqItems.flatMap((r,i) =>
          r.equipmentUnits.length
            ? r.equipmentUnits.map((u,ui) => [
                `${i+1}.${ui+1}`, r.itemCode,
                (r.description||r.itemName).substring(0,30), "1","—",
                [u.serialNumber, u.macAddress, u.unitAddress].filter(Boolean).join("|").substring(0,30),
              ])
            : [[String(i+1), r.itemCode, (r.description||r.itemName).substring(0,36), String(r.requestedQty),"—","Sin pistolear"]]
        )
      );
    }

    if (y>230) { doc.addPage(); y=15; }
    y+=8; doc.setDrawColor(200,200,200); doc.line(M,y,W-M,y); y+=10;
    doc.setFontSize(9); doc.setFont("helvetica","bold"); doc.setTextColor(50,50,50);
    doc.text("FIRMAS Y CONFORMIDAD", M, y); y+=10;
    const bw=(CW-10)/2;
    doc.setDrawColor(180,180,180);
    doc.rect(M,y,bw,28,"S");
    doc.setFont("helvetica","normal"); doc.setFontSize(8);
    doc.text("ENTREGADO POR:", M+3,y+6); doc.setFont("helvetica","bold");
    doc.text("ALMACÉN LPS GROUP", M+3, y+12);
    doc.setFont("helvetica","normal"); doc.setFontSize(7);
    doc.text("Firma: ___________________________", M+3, y+26);
    doc.rect(M+bw+10,y,bw,28,"S");
    doc.setFont("helvetica","normal"); doc.setFontSize(8);
    doc.text("RECIBIDO POR:", M+bw+13, y+6); doc.setFont("helvetica","bold");
    doc.text(selectedWorker.name, M+bw+13, y+12);
    doc.setFont("helvetica","normal"); doc.setFontSize(7);
    doc.text(`DNI: ${selectedWorker.dni}`, M+bw+13, y+18);
    doc.text("Firma: ___________________________", M+bw+13, y+26);
    ...
    for (let p=1;p<=pages;p++) {
      ...
      doc.text(`LPS GROUP S.A.C. — LPS ERP — ${dayjs().format("DD/MM/YYYY HH:mm")}`, M, 292);
      doc.text(`Pág. ${p}/${pages}`, W-M-14, 292);
    }
    doc.save(`Despacho_${dispatchCode}_${selectedWorker.dni}.pdf`);
  };

  // ─── RENDER ───────────────────────────────────────────────────────────────
  const cfg = P[activeTab];
  ...
  return (
    <Box ref={topRef} sx={{ maxWidth: 1400, mx: "auto", p: 3, display: "flex", flexDirection: "column", gap: 3 }}>
      <TitleCard
        icon={<LocalShippingOutlined sx={{ fontSize: 32 }} />}
        title="Despacho a Cuadrillas"
        description="Ingresa las cantidades directamente en la grilla. Ítems con cantidad > 0 forman el despacho."
      />
      ...
      <Fade in={showSuccess} timeout={700} unmountOnExit>
        <Box>
          <Card elevation={0} sx={{
            ...
            <Typography variant="h4" fontWeight={800} sx={{ color:"#15803d",mb:1 }}>¡Despacho Registrado!</Typography>
            <Typography variant="h6" color="text.secondary" sx={{ mb:0.5,fontWeight:400 }}>{selectedWorker?.name}</Typography>
            <Typography variant="body2" color="text.disabled" sx={{ mb:4 }}>DNI: {selectedWorker?.dni}</Typography>
            <Box sx={{ display:"flex",gap:2,justifyContent:"center",flexWrap:"wrap",mb:4 }}>
              {[
                { label:"Código",   value: dispatchCode, color:"#0369a1" },
                { label:"Ítems",    value: totalItems,   color:"#ea580c" },
                { label:"Unidades", value: totalUnits,   color:"#15803d" },
              ].map(c => (
                <Paper key={c.label} elevation={0} sx={{ px:3,py:2,borderRadius:3,border:"1px solid #bbf7d0",bgcolor:"white",minWidth:110,textAlign:"center" }}>
            ...
            <Stack direction="row" spacing={2} justifyContent="center" flexWrap="wrap">
              <ButtonBase label="Descargar Cargo PDF" startIcon={<PictureAsPdfOutlined />} onClick={handlePDF}
                sx={{ px:4,py:1.5,bgcolor:"#dc2626","&:hover":{bgcolor:"#b91c1c"},boxShadow:"0 4px 12px rgba(220,38,38,0.3)",borderRadius:2.5 }}
              />
              <ButtonBase label="Nuevo Despacho" startIcon={<LocalShippingOutlined />} onClick={handleReset}
                sx={{ px:4,py:1.5,fontWeight:700,boxShadow:"0 4px 12px rgba(34,197,94,0.3)",borderRadius:2.5 }}
              />
            </Stack>
          </Card>
      ...
      <Fade in={!showSuccess} timeout={500} unmountOnExit>
        <Box sx={{ display:"flex",flexDirection:"column",gap:3 }}>
              ...
              <Box sx={{ flex:"0 0 400px",bgcolor:"#f8fafc",p:3.5,borderRight:{ md:"1px solid #e2e8f0" } }}>
                <Stack spacing={2.5}>
                  <Box sx={{ display:"flex",alignItems:"center",gap:1.5 }}>
                    ...
                    {loadingW && <CircularProgress size={14} />}
                  </Box>
                  <Autocomplete
                    options={workers}
                    getOptionLabel={w => `${w.name} — ${w.dni}`}
                    ...
                    loading={loadingW}
                    renderInput={params => (
                      <TextField {...params} size="small" placeholder="Buscar por nombre o DNI..."
                        InputProps={{ ...params.InputProps, startAdornment:<><SearchOutlined sx={{ fontSize:18,color:"text.disabled",mr:0.5 }} />{params.InputProps.startAdornment}</> }}
                      />
                    )}
                    renderOption={(props,option) => (
                      <Box component="li" {...props} sx={{ py:1.5 }} key={option.dni}>
                        <Stack direction="row" spacing={1.5} alignItems="center">
                          ...
                          <Box>
                            ...
                            <Stack direction="row" spacing={0.5} alignItems="center" mt={0.3}>
                              <Typography variant="caption" color="text.disabled">{option.dni}</Typography>
                              <Chip label={ROLE_LABEL[option.role]??option.role} size="small" color={ROLE_COLOR[option.role]??"default"} sx={{ height:16,fontSize:"0.65rem",fontWeight:700 }} />
                            </Stack>
                          </Box>
                        </Stack>
                      </Box>
                    )}
                    noOptionsText="No se encontraron trabajadores" fullWidth
                  />
                  <Box sx={{ display:"flex",gap:2 }}>
                    <Box sx={{ flex:1 }}>
                      <Typography variant="caption" color="text.secondary" fontWeight={600} display="block" mb={0.5}>Fecha Programada</Typography>
                      <TextField type="date" fullWidth size="small" value={scheduledDate} onChange={e => setScheduledDate(e.target.value)} InputLabelProps={{ shrink:true }} />
                    </Box>
                    <Box sx={{ flex:"0 0 110px" }}>
                      <Typography variant="caption" color="text.secondary" fontWeight={600} display="block" mb={0.5}>Prioridad</Typography>
                      <ToggleButtonGroup value={priority} exclusive size="small" onChange={(_,v) => { if(v) setPriority(v); }} fullWidth>
                        {(["HIGH","MEDIUM","LOW"] as const).map(p => (
                          <ToggleButton key={p} value={p} sx={{ textTransform:"none",fontSize:"0.7rem",fontWeight:700,
                            ...(priority===p&&p==="HIGH"&&{bgcolor:"#fee2e2 !important",color:"#dc2626 !important",borderColor:"#dc2626 !important"}),
                            ...(priority===p&&p==="MEDIUM"&&{bgcolor:"#fef9c3 !important",color:"#ca8a04 !important",borderColor:"#ca8a04 !important"}),
                            ...(priority===p&&p==="LOW"&&{bgcolor:"#f0fdf4 !important",color:"#16a34a !important",borderColor:"#16a34a !important"}),
                          }}>
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary" fontWeight={600} display="block" mb={0.5}>Observaciones</Typography>
                    <TextField fullWidth size="small" multiline rows={2} placeholder="Instrucciones del despacho..." value={observations} onChange={e => setObservations(e.target.value)} />
                  </Box>
                </Stack>
              </Box>
              ...
              <Box sx={{ flex:1,p:3.5,display:"flex",alignItems:"center" }}>
                {selectedWorker ? (
                  <Fade in timeout={400}>
                    <Box sx={{ width:"100%" }}>
                      <Stack direction="row" spacing={2.5} alignItems="center" mb={3}>
                        <Avatar sx={{ width:68,height:68,fontSize:26,fontWeight:800,bgcolor:selectedWorker.role==="JEFE_CUADRILLA"?"#d97706":"#1976d2",boxShadow:"0 4px 14px rgba(0,0,0,0.15)" }}>
                          {selectedWorker.name.charAt(0)}
                        </Avatar>
                        <Box>
                          <Typography variant="h6" fontWeight={800} sx={{ lineHeight:1.2 }}>{selectedWorker.name}</Typography>
                          <Stack direction="row" spacing={1} mt={0.5}>
                            <Chip label={ROLE_LABEL[selectedWorker.role]??selectedWorker.role} size="small" color={ROLE_COLOR[selectedWorker.role]??"default"} sx={{ fontWeight:700 }} />
                            <Chip label={selectedWorker.type} size="small" variant="outlined" sx={{ fontWeight:600 }} />
                          </Stack>
                        </Box>
                      </Stack>
                      <Box sx={{ display:"flex",gap:3,flexWrap:"wrap" }}>
                        ...
                        <Box sx={{ display:"flex",alignItems:"center",gap:1 }}>
                          <CalendarTodayOutlined sx={{ fontSize:18,color:"text.secondary" }} />
                          <Typography variant="body2" color="text.secondary">Fecha:</Typography>
                          <Typography variant="body2" fontWeight={700}>{dayjs(scheduledDate).format("DD [de] MMMM, YYYY")}</Typography>
                        </Box>
                      </Box>
                      {totalItems > 0 && (
                        <Box sx={{ mt:2.5,p:2,borderRadius:2,bgcolor:"#f0f9ff",border:"1px solid #bae6fd",display:"flex",gap:3,alignItems:"center",flexWrap:"wrap" }}>
                          <Box>
                            <Typography variant="caption" color="text.secondary" fontWeight={600}>Ítems</Typography>
                            <Typography variant="h6" fontWeight={800} sx={{ color:"#0369a1",lineHeight:1 }}>{totalItems}</Typography>
                          </Box>
                          <Box>
                            <Typography variant="caption" color="text.secondary" fontWeight={600}>Unidades</Typography>
                            <Typography variant="h6" fontWeight={800} sx={{ color:"#0369a1",lineHeight:1 }}>{totalUnits}</Typography>
                          </Box>
                          {(["MATERIAL","TOOL","EQUIPMENT","EPP"] as ProductTab[]).map(t => {
                            const count = selectedItems.filter(r => r.productType===t).length;
                            if (!count) return null;
                            const c = P[t];
                            return <Chip key={t} label={`${c.icon} ${count}`} size="small" sx={{ fontWeight:800,bgcolor:c.bg,color:c.color,border:`1px solid ${c.border}` }} />;
                          })}
                        </Box>
                      )}
                    </Box>
                  ...
                  <Box sx={{ width:"100%",textAlign:"center",py:4 }}>
                    <PersonOutlined sx={{ fontSize:56,color:"text.disabled",mb:1.5 }} />
                    <Typography variant="body1" color="text.secondary" fontWeight={600}>Selecciona un técnico para continuar</Typography>
                    <Typography variant="caption" color="text.disabled">Busca por nombre completo o DNI</Typography>
                  </Box>
                )}
              </Box>
            </Box>
          ...
          <Card elevation={3} sx={{ borderRadius:4,boxShadow:"rgba(149,157,165,0.2) 0px 8px 24px",overflow:"hidden" }}>
            ...
            <Box sx={{ px:3,py:2,bgcolor:"#fafbfc",borderBottom:"1px solid #f1f5f9",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:2 }}>
              <Stack direction="row" spacing={2} alignItems="center">
                <Box sx={{ width:4,height:22,bgcolor:cfg.color,borderRadius:1 }} />
                <Box>
                  <Typography variant="subtitle1" fontWeight={800}>Catálogo de Inventario</Typography>
                  <Typography variant="caption" color="text.secondary">
                    Ajusta la cantidad por ítem — los de cantidad &gt; 0 se incluyen en el despacho
                  </Typography>
                </Box>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
                <TextField size="small" placeholder="Buscar ítem..." value={invSearch} onChange={e => setInvSearch(e.target.value)} sx={{ width:220 }}
                ...
                <Tooltip title="Recargar">
                  <span>
                    <IconButton size="small" onClick={() => setInvPage(0)} sx={{ border:"1px solid #e2e8f0",bgcolor:"white" }}>
                      <RefreshOutlined fontSize="small" />
                    </IconButton>
                  </span>
                </Tooltip>
            ...
            {/* Tabs */}
            <Box sx={{ borderBottom:"1px solid #f1f5f9",px:3,bgcolor:"#fafbfc" }}>
              <Tabs value={activeTab} onChange={(_,v) => { setActiveTab(v); setInvPage(0); setInvSearch(""); }}
                TabIndicatorProps={{ sx:{ height:3,borderRadius:"3px 3px 0 0" } }}>
                {(["MATERIAL","TOOL","EQUIPMENT","EPP"] as ProductTab[]).map(type => {
                  const c = P[type];
                  const count = selectedItems.filter(r => r.productType===type).length;
                  return (
                    <Tab key={type} value={type}
                      label={
                        <Box sx={{ display:"flex",alignItems:"center",gap:0.8 }}>
                          <Typography variant="caption" fontWeight={700}>{c.icon} {c.label}</Typography>
                          {count>0 && <Chip label={count} size="small" sx={{ height:18,fontSize:"0.65rem",fontWeight:800,bgcolor:c.color,color:"white" }} />}
                        </Box>
                      }
                      sx={{ textTransform:"none",minHeight:44 }}
                    />
            ...
            <Box sx={{ p:2 }}>
              {loadingInv ? (
                ...
                <Box sx={{ textAlign:"center",py:8 }}>
                  <StorageOutlined sx={{ fontSize:48,color:"text.disabled",mb:1 }} />
                  <Typography variant="body2" color="text.secondary">Sin ítems para este tipo</Typography>
                </Box>
              ) : (
                <>
                  {/* Column headers */}
                  <Box sx={{ display:"grid", gridTemplateColumns:"24px 105px 1fr 80px 76px 116px 90px", gap:1, px:1.5, py:1, bgcolor:"#f1f5f9", borderRadius:1, mb:0.5 }}>
                    {["","CÓDIGO","DESCRIPCIÓN","DISP.","ESTADO","CANTIDAD",""].map((h,i) => (
                      <Typography key={i} variant="caption" fontWeight={700} color="text.secondary"
                        sx={{ textAlign: i >= 3 ? "center" : "left" }}>{h}</Typography>
                    ))}
                  </Box>

                  <Box sx={{ display:"flex",flexDirection:"column",gap:0.4,maxHeight:430,overflowY:"auto",pr:0.5,
                    "&::-webkit-scrollbar":{ width:4 }, "&::-webkit-scrollbar-thumb":{ bgcolor:"#cbd5e1",borderRadius:2 } }}>
                    {rows.map(row => {
                      const sem    = getSem(row);
                      const hasQty = row.requestedQty > 0;
                      const isEq   = row.productType === "EQUIPMENT";
                      const pistolados = row.equipmentUnits.length;
                      const pct  = Math.min((row.quantityAvailable/(row.maximumStock||Math.max(row.quantityAvailable,1)))*100,100);

                      return (
                        <Box key={row.id} sx={{
                          display:"grid", gridTemplateColumns:"24px 105px 1fr 80px 76px 116px 90px",
                          gap:1, alignItems:"center", px:1.5, py:1,
                          borderRadius:1.5, border:"1px solid",
                          bgcolor: hasQty ? cfg.bg : "#fafafa",
                          borderColor: hasQty ? cfg.color+"50" : "#f0f0f0",
                          transition:"all 0.12s",
                          opacity: sem==="GRIS" ? 0.5 : 1,
                        }}>
                          {/* Semaforo */}
                          <Box sx={{ display:"flex",justifyContent:"center" }}>
                            <Box sx={{ width:8,height:8,borderRadius:"50%",bgcolor:SEM[sem].color,
                              ...(sem==="ROJO"&&{ animation:"sp 1.6s ease-in-out infinite",
                                "@keyframes sp":{"0%,100%":{boxShadow:`0 0 0 0 ${SEM[sem].color}60`},"50%":{boxShadow:`0 0 0 4px ${SEM[sem].color}00`}} }) }} />
                          </Box>

                          {/* Código */}
                          <Chip label={row.itemCode} size="small"
                            sx={{ bgcolor:cfg.color,color:"white",fontWeight:700,fontSize:"0.65rem",maxWidth:100 }} />

                          {/* Descripción */}
                          <Typography variant="caption" fontWeight={hasQty?700:400}
                            sx={{ lineHeight:1.3, color: hasQty ? cfg.color : "text.primary" }}>
                            {row.description || row.itemName}
                          </Typography>

                          {/* Disponible */}
                          <Box sx={{ textAlign:"center" }}>
                            <Typography variant="caption" fontWeight={800} sx={{ color:SEM[sem].color }}>{row.quantityAvailable}</Typography>
                            <LinearProgress variant="determinate" value={pct} sx={{ height:3,borderRadius:2,mt:0.3,
                              bgcolor:`${SEM[sem].color}20`, "& .MuiLinearProgress-bar":{ bgcolor:SEM[sem].color,borderRadius:2 } }} />
                          </Box>

                          {/* Estado */}
                          <Box sx={{ textAlign:"center" }}>
                            <Chip label={SEM[sem].label} size="small" sx={{ height:18,fontSize:"0.6rem",fontWeight:700,
                              bgcolor:`${SEM[sem].color}15`,color:SEM[sem].color }} />
                          </Box>

                          {/* Cantidad con stepper */}
                          <Box sx={{ display:"flex",alignItems:"center",gap:0.3,justifyContent:"center" }}>
                            <IconButton size="small" sx={{ p:0.3 }}
                              onClick={() => setRowQty(row.id, row.requestedQty-1)}
                              disabled={row.requestedQty===0}>
                              <RemoveOutlined sx={{ fontSize:14,color:row.requestedQty===0?"#d1d5db":cfg.color }} />
                            </IconButton>
                            <TextField
                              type="number"
                              value={row.requestedQty===0 ? "" : row.requestedQty}
                              placeholder="0"
                              onChange={e => { const v=parseInt(e.target.value); setRowQty(row.id, isNaN(v)?0:v); }}
                              inputProps={{ min:0,max:row.quantityAvailable,style:{ textAlign:"center",fontWeight:800,padding:"3px 0",fontSize:"0.82rem",width:"32px" } }}
                              sx={{ "& .MuiOutlinedInput-root":{ borderRadius:1.5,fontSize:"0.82rem",
                                ...(hasQty&&{"& fieldset":{ borderColor:cfg.color,borderWidth:2 }}) },
                            ...
                            <IconButton size="small" sx={{ p:0.3 }}
                              onClick={() => setRowQty(row.id, row.requestedQty+1)}
                              disabled={row.requestedQty>=row.quantityAvailable||sem==="GRIS"}>
                              <AddOutlined sx={{ fontSize:14,color:row.requestedQty>=row.quantityAvailable?"#d1d5db":cfg.color }} />
                            </IconButton>
                          </Box>
                          ...
                          <Box sx={{ display:"flex",justifyContent:"center" }}>
                            {isEq && hasQty ? (
                              <Tooltip title={`${pistolados}/${row.requestedQty} pistoleados — clic para pistolear`}>
                                <Box onClick={() => openPistoleo(row)}
                                  sx={{ display:"flex",alignItems:"center",gap:0.5,px:1,py:0.4,borderRadius:1.5,cursor:"pointer",
                                    bgcolor:pistolados===row.requestedQty?"#f0fdf4":"#fffbeb",
                                    border:`1px solid ${pistolados===row.requestedQty?"#bbf7d0":"#fde68a"}`,
                                    "&:hover":{ transform:"scale(1.04)" },transition:"all 0.15s",
                                  }}>
                                  <QrCodeScannerOutlined sx={{ fontSize:13,color:pistolados===row.requestedQty?"#15803d":"#b45309" }} />
                                  <Typography variant="caption" fontWeight={800}
                                    sx={{ color:pistolados===row.requestedQty?"#15803d":"#b45309",fontSize:"0.65rem" }}>
                                    {pistolados}/{row.requestedQty}
                                  </Typography>
                                </Box>
                              </Tooltip>
                            ) : hasQty ? (
                  ...
                  {invTotal > 25 && (
                    <Box sx={{ display:"flex",justifyContent:"center",alignItems:"center",gap:2,mt:2 }}>
                      <ButtonBase size="small" label="← Ant." disabled={invPage===0} onClick={() => setInvPage(p=>p-1)}
                        sx={{ bgcolor:"white",color:"text.secondary",border:"1px solid #e2e8f0",fontSize:"0.75rem" }} />
                      <Typography variant="caption" color="text.secondary">
                        Pág. {invPage+1} / {Math.ceil(invTotal/25)} ({invTotal})
                      </Typography>
                      <ButtonBase size="small" label="Sig. →" disabled={(invPage+1)*25>=invTotal} onClick={() => setInvPage(p=>p+1)}
                        sx={{ bgcolor:"white",color:"text.secondary",border:"1px solid #e2e8f0",fontSize:"0.75rem" }} />
                    </Box>
                  )}
                </>
          ...
          <Box sx={{ display:"flex",gap:2,justifyContent:"flex-end",flexWrap:"wrap",alignItems:"center" }}>
            {totalItems > 0 && (
              <Box sx={{ px:3,py:1.2,borderRadius:2,bgcolor:"#f0f9ff",border:"1px solid #bae6fd" }}>
                <Typography variant="body2" fontWeight={700} sx={{ color:"#0369a1" }}>
                  {totalItems} ítem{totalItems>1?"s":""} · {totalUnits} unidades seleccionadas
                </Typography>
              </Box>
            )}
            <ButtonBase label="Revisar y Confirmar Despacho" startIcon={<SendOutlined />}
              onClick={() => {
                if (!selectedWorker) { toast.warning("Selecciona un técnico"); return; }
                if (totalItems===0) { toast.warning("Ingresa cantidad en al menos un ítem"); return; }
                setConfirmOpen(true);
              }}
              disabled={!selectedWorker || totalItems===0}
      ...
      <Dialog open={pistoleoOpen} onClose={() => setPistoleoOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogTitle sx={{ m:0,p:0 }}>
          <Box sx={{ bgcolor:"#1e293b",p:3,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Box>
                <Typography variant="subtitle1" fontWeight={700} color="white">Pistoleo de Equipos</Typography>
                {pistoleoRow && (
                  <Typography variant="caption" sx={{ color:"rgba(255,255,255,0.6)" }}>
                    {pistoleoRow.itemCode} — {(pistoleoRow.description||pistoleoRow.itemName).substring(0,50)}
                  </Typography>
                )}
              </Box>
            </Stack>
            ...
            <>
              ...
              <Box sx={{ mb:3,p:2.5,bgcolor:"#f8fafc",borderRadius:2,border:"1px solid #e2e8f0" }}>
                <Box sx={{ display:"flex",justifyContent:"space-between",alignItems:"center",mb:1.5 }}>
                  ...
                  <Chip
                    label={`${pistoleoRow.equipmentUnits.length} / ${pistoleoRow.requestedQty}`}
                    size="small"
                    color={pistoleoRow.equipmentUnits.length===pistoleoRow.requestedQty?"success":pistoleoRow.equipmentUnits.length>0?"warning":"default"}
                    sx={{ fontWeight:800 }}
                  />
                </Box>
                <LinearProgress variant="determinate"
                  value={Math.min((pistoleoRow.equipmentUnits.length/pistoleoRow.requestedQty)*100,100)}
                  sx={{ height:8,borderRadius:4,bgcolor:"#e2e8f0",
                    "& .MuiLinearProgress-bar":{ bgcolor:pistoleoRow.equipmentUnits.length===pistoleoRow.requestedQty?"#22c55e":"#f59e0b",borderRadius:4 } }}
                />
                <Typography variant="caption" color="text.secondary" mt={1} display="block">
                  Registra <strong>{pistoleoRow.requestedQty}</strong> equipo(s). Puedes confirmar el despacho con pistolado parcial.
                </Typography>
              </Box>

              {/* Formulario nueva unidad */}
              {pistoleoRow.equipmentUnits.length < pistoleoRow.requestedQty && (
                <Box sx={{ mb:3,p:2.5,bgcolor:"#f0fdf4",borderRadius:2,border:"1px solid #bbf7d0" }}>
                  <Typography variant="body2" fontWeight={700} mb={1.5} sx={{ display:"flex",alignItems:"center",gap:1,color:"#15803d" }}>
                    <AddOutlined sx={{ fontSize:16 }} />
                    Equipo #{pistoleoRow.equipmentUnits.length+1} de {pistoleoRow.requestedQty}
                  </Typography>
                  <Box sx={{ display:"flex",flexDirection:"column",gap:1.2 }}>
                    <TextField size="small" label="N° Serie *" placeholder="Ej: ZTEATV45501950107" fullWidth
                      value={editUnit.serialNumber??""} onChange={e => setEditUnit(p=>({...p,serialNumber:e.target.value}))}
                      onKeyDown={e => { if(e.key==="Enter") addUnit(); }}
                      InputProps={{ startAdornment:<InputAdornment position="start"><QrCodeScannerOutlined sx={{ fontSize:16,color:"text.disabled" }} /></InputAdornment> }}
                    />
                    <Box sx={{ display:"flex",gap:1,flexWrap:"wrap" }}>
                      <TextField size="small" label="MAC Address" placeholder="6C:B8:81:F2:B7:D7" sx={{ flex:"1 1 150px" }}
                        value={editUnit.macAddress??""} onChange={e => setEditUnit(p=>({...p,macAddress:e.target.value}))} />
                      <TextField size="small" label="MTA MAC" placeholder="CC00F1CA6351" sx={{ flex:"1 1 130px" }}
                        value={editUnit.mtaMacAddress??""} onChange={e => setEditUnit(p=>({...p,mtaMacAddress:e.target.value}))} />
                      <TextField size="small" label="Unit Address (UA)" placeholder="12345678" sx={{ flex:"1 1 130px" }}
                        value={editUnit.unitAddress??""} onChange={e => setEditUnit(p=>({...p,unitAddress:e.target.value}))} />
                    </Box>
                    <ButtonBase fullWidth label={`Agregar equipo #${pistoleoRow.equipmentUnits.length+1}`}
                      startIcon={<AddOutlined />} onClick={addUnit}
                      disabled={!editUnit.serialNumber?.trim()}
                      sx={{ py:1,background:"linear-gradient(135deg,#15803d 0%,#16a34a 100%)",boxShadow:"0 4px 12px rgba(21,128,61,0.25)" }}
                    />
                  </Box>
                </Box>
              )}
              ...
              {pistoleoRow.equipmentUnits.length > 0 && (
                <Box>
                  <Typography variant="body2" fontWeight={700} mb={1.2} sx={{ display:"flex",alignItems:"center",gap:1 }}>
                    ...
                    Equipos registrados ({pistoleoRow.equipmentUnits.length})
                  </Typography>
                  <Stack spacing={0.7}>
                    {pistoleoRow.equipmentUnits.map((unit,idx) => (
                      <Box key={unit.unitId} sx={{ p:1.5,bgcolor:"#f0fdf4",borderRadius:2,border:"1px solid #bbf7d0",
                        display:"flex",alignItems:"flex-start",justifyContent:"space-between" }}>
                        <Stack spacing={0.3}>
                          <Chip label={`#${idx+1}`} size="small" color="success" sx={{ fontWeight:800,height:18,fontSize:"0.65rem",width:"fit-content" }} />
                          <Typography variant="caption" fontWeight={700}>SN: {unit.serialNumber}</Typography>
                          {unit.macAddress    && <Typography variant="caption" color="text.secondary">MAC: {unit.macAddress}</Typography>}
                          {unit.mtaMacAddress && <Typography variant="caption" color="text.secondary">MTA: {unit.mtaMacAddress}</Typography>}
                          {unit.unitAddress   && <Typography variant="caption" color="text.secondary">UA: {unit.unitAddress}</Typography>}
                        </Stack>
                        <IconButton size="small" sx={{ color:"error.light",mt:-0.5 }}
                          onClick={() => removeUnit(pistoleoRow.id, unit.unitId)}>
        ...
        <DialogActions sx={{ p:2.5,bgcolor:"#f8fafc",borderTop:"1px solid #e2e8f0" }}>
          <ButtonBase label="Cerrar" onClick={() => setPistoleoOpen(false)}
            sx={{ bgcolor:"white",color:"#64748b",border:"1px solid #cbd5e1" }} />
          {pistoleoRow && pistoleoRow.equipmentUnits.length === pistoleoRow.requestedQty && (
            <ButtonBase label="✓ Listo — Todos pistoleados" onClick={() => setPistoleoOpen(false)}
              sx={{ px:3,bgcolor:"#15803d" }} />
          )}
        </DialogActions>
      </Dialog>

      {/* ══ MODAL CONFIRMACIÓN ═══════════════════════════════════════════════ */}
      <Dialog open={confirmOpen} onClose={() => !submitting && setConfirmOpen(false)} maxWidth="md" fullWidth
        ...
        <DialogTitle sx={{ m:0,p:0 }}>
          <Box sx={{ background:"linear-gradient(135deg,#1d4ed8 0%,#2563eb 100%)",p:3,color:"white",
            ...
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Box>
                <Typography variant="h6" fontWeight={800}>Confirmar Despacho</Typography>
                <Typography variant="caption" sx={{ opacity:0.85 }}>
                  {totalItems} ítem{totalItems>1?"s":""} · {totalUnits} unidades
                </Typography>
              </Box>
            </Stack>
            {!submitting && (
        ...
        <DialogContent sx={{ p:3 }}>
          {/* Resumen técnico */}
          <Box sx={{ p:2,mb:2,bgcolor:"#f8fafc",borderRadius:2,border:"1px solid #e2e8f0",display:"flex",gap:3,flexWrap:"wrap" }}>
            <Box>
              <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ textTransform:"uppercase" }}>Técnico</Typography>
              <Typography variant="body1" fontWeight={800}>{selectedWorker?.name}</Typography>
            </Box>
            <Box>
              <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ textTransform:"uppercase" }}>DNI</Typography>
              <Typography variant="body1" fontWeight={700}>{selectedWorker?.dni}</Typography>
            </Box>
            <Box>
              <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ textTransform:"uppercase" }}>Fecha</Typography>
              <Typography variant="body1" fontWeight={700}>{dayjs(scheduledDate).format("DD/MM/YYYY")}</Typography>
            </Box>
            <Box>
              <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ textTransform:"uppercase" }}>Prioridad</Typography>
              <Chip label={priority} size="small" color={priority==="HIGH"?"error":priority==="MEDIUM"?"warning":"success"} sx={{ fontWeight:700 }} />
            </Box>
          </Box>

          {/* Advertencias equipo sin pistolear */}
          {equipmentWarnings.length > 0 && (
            <Alert severity="warning" sx={{ mb:2,borderRadius:2 }}>
              <Typography variant="body2" fontWeight={700} mb={0.5}>Equipos con pistolado incompleto:</Typography>
              {equipmentWarnings.map((w,i) => (
                <Typography key={i} variant="caption" display="block" color="text.secondary">• {w}</Typography>
              ))}
              <Typography variant="caption" color="text.secondary" mt={0.5} display="block">
                Puedes confirmar de todas formas — solo se envían las unidades pistoleadas.
              </Typography>
            </Alert>
          )}
            ...
            {(["MATERIAL","TOOL","EQUIPMENT","EPP"] as ProductTab[]).map(type => {
              ...
              return (
                <Box key={type} sx={{ mb:2 }}>
                  <Box sx={{ px:2,py:0.8,bgcolor:c.bg,border:`1px solid ${c.border}`,borderRadius:1.5,mb:1,display:"flex",alignItems:"center",gap:1 }}>
                    <Typography variant="body2" fontWeight={800} sx={{ color:c.color }}>{c.icon} {c.label}</Typography>
                    <Chip label={items.length} size="small" sx={{ height:18,fontSize:"0.65rem",fontWeight:800,bgcolor:c.color,color:"white" }} />
                  </Box>
                  {items.map((row,i) => (
                    <Box key={row.id} sx={{ display:"flex",gap:1.5,alignItems:"center",px:1.5,py:1,
                      bgcolor:i%2===0?"#fafafa":"white",borderRadius:1,mb:0.4,border:"1px solid #f0f0f0" }}>
                      <Chip label={row.itemCode} size="small" sx={{ bgcolor:c.color,color:"white",fontWeight:700,fontSize:"0.62rem",flexShrink:0 }} />
                      <Typography variant="caption" fontWeight={600} sx={{ flex:1,lineHeight:1.3 }}>
                        {row.description||row.itemName}
                      </Typography>
                      {row.productType === "EQUIPMENT" ? (
                        <Stack direction="row" spacing={0.5} alignItems="center">
                          <Chip label={`${row.equipmentUnits.length} ud.`} size="small"
                            color={row.equipmentUnits.length===row.requestedQty?"success":"warning"}
                            sx={{ height:20,fontWeight:800,fontSize:"0.68rem" }} />
                          {row.equipmentUnits.length < row.requestedQty && (
                            <Chip label={`${row.requestedQty-row.equipmentUnits.length} sin serie`} size="small"
                              color="warning" variant="outlined" sx={{ height:20,fontWeight:700,fontSize:"0.65rem" }} />
                          )}
                        </Stack>
                      ) : (
                        <Chip label={`${row.requestedQty} ${row.uom||"ud."}`} size="small"
                          sx={{ height:20,fontWeight:800,fontSize:"0.68rem",bgcolor:c.bg,color:c.color,border:`1px solid ${c.border}` }} />
                      )}
                    </Box>
                  ))}
                </Box>
          ...
          <Alert severity="info" sx={{ mt:2,borderRadius:2 }}>
            Al confirmar se registrará el despacho en <strong>POST /api/dispatches</strong>. Esta acción es irreversible.
          </Alert>
        </DialogContent>

        <DialogActions sx={{ p:3,bgcolor:"#f8fafc",borderTop:"1px solid #e2e8f0",gap:1 }}>
          <ButtonBase label="Volver y editar" onClick={() => setConfirmOpen(false)} disabled={submitting}
            sx={{ bgcolor:"white",color:"#64748b",border:"1px solid #cbd5e1" }} />
          <ButtonBase
            label={submitting?"Registrando...":"Confirmar y Registrar Despacho"}
            startIcon={submitting?<CircularProgress size={18} color="inherit" />:<CheckCircleOutline />}
            onClick={handleSubmit}
            disabled={submitting}
