...
// //   ReceiptLongOutlined, StorageOutlined, ErrorOutlined,
...
// // // ─── Helpers para construir los payloads ───────────────────────────────────
...
// //   items.forEach(item => {
// //     const qty = item.receivedQuantity ?? item.pendingQuantity;
// //     if (!qty || qty <= 0) return;

// //     if (item.productType === "EQUIPMENT") {
// //       // Un movement por serial
...
// //           notes: `Recepción solicitud #${supplyRequestId} — serial ${serial.serialNumber}`,
// //           referenceType: "SUPPLY_REQUEST",
// //           referenceId: supplyRequestId,
// //           serialNumber: serial.serialNumber,
// //           createdBy,
// //         });
// //       });
// //     } else {
// //       // Materiales y herramientas: un movement por item
// //       movements.push({
// //         inventoryId: item.inventoryId,
// //         itemId: item.itemId,
// //         movementType: "ENTRY",
// //         quantity: qty,
// //         unitPrice: item.unitPrice ?? 0,
// //         notes: `Recepción solicitud #${supplyRequestId}`,
// //         referenceType: "SUPPLY_REQUEST",
// //         referenceId: supplyRequestId,
// //         createdBy,
// //       });
...
// //     quantityDelivered: item.receivedQuantity ?? item.pendingQuantity,
// //     unitPrice: item.unitPrice ?? 0,
// //   }));
...
// //   const [procesSteps, setProcessSteps]  = useState<ProcessStep[]>([
// //     { label: "Kardex de Entrada",     description: "Registrando movimientos de entrada en el kardex...", status: "idle" },
// //     { label: "Inventario y Equipos",  description: "Actualizando stock y registrando equipment units...",  status: "idle" },
// //   ]);
...
// //   // ── Scroll al stepper cuando aparece ──────────────────────────────────
// //   useEffect(() => {
// //     if (showStepper) {
// //       setTimeout(() => {
...
// //     setActiveStep(0);
// //     setProcessSteps(steps => steps.map(s => ({ ...s, status: "idle", result: undefined, error: undefined })));
// //   };
...
// //     const regla = validacionesCampo[field];
// //     if (regla && value.trim()) {
// //       setCamposError(prev => ({ ...prev, [field]: validarCampo(field, value) }));
// //     } else {
...
// //     try {
// //       const kardexPayload = buildKardexPayload(itemsRecepcion, supplyRequestId);
...
// //       if (!resKardex.ok) {
// //         const err = await resKardex.json().catch(() => ({}));
// //         throw new Error(err.message ?? `HTTP ${resKardex.status}`);
// //       }
...
// //       updateStep(0, {
// //         status: "success",
// //         result: `${kardexMovementsCount} movimientos registrados (IDs ${kardexResult.insertedIds?.[0]} → ${kardexResult.insertedIds?.at(-1)})`,
// //       });
// //       setActiveStep(1);
// //     } catch (e) {
// //       updateStep(0, { status: "error", error: e instanceof Error ? e.message : "Error en kardex-batch" });
// //       return; // No continuar si falla el kardex
// //     }

// //     // ── STEP 2: inventory-batch ──────────────────────────────────────
...
// //       if (!resInventory.ok) {
// //         const err = await resInventory.json().catch(() => ({}));
// //         throw new Error(err.message ?? `HTTP ${resInventory.status}`);
// //       }
...
// //       updateStep(1, {
// //         status: "success",
// //         result: `${inventoryResult.inventoryLinesUpdated} inventarios actualizados, ${equipmentUnitsCount} equipos registrados`,
// //       });
...
// //         setSuccessData({
// //           id: supplyRequestId,
// //           totalItems: itemsRecepcion.length,
...
// //     } catch (e) {
// //       updateStep(1, { status: "error", error: e instanceof Error ? e.message : "Error en inventory-batch" });
// //     }
// //   };
...
// //     {
// //       field: "deliveredQuantity", headerName: "A Recibir", width: 120,
...
// //         {
// //       field: "deliveredQuantity", headerName: "Recibido", width: 120,
// //       editable: true, type: "number", align: "center", headerAlign: "center",
// //       renderCell: p => (
// //         <Typography variant="body2" fontWeight={700}
// //           sx={{ color: p.value === p.row.requestedQuantity ? "success.main" : "warning.main" }}>
...
// //                           {step.status === "idle"    && (
// //                             <Typography variant="caption" fontWeight={700} sx={{ color: index <= activeStep ? "#4f46e5" : "#94a3b8" }}>
// //                               {index + 1}
// //                             </Typography>
// //                           )}
// //                         </Box>
...
// //                         sx={{ color: step.status === "error" ? "error.main" : step.status === "success" ? "success.main" : "text.primary" }}>
// //                         {step.label}
// //                       </Typography>
// //                     </StepLabel>
...
// //                               El proceso se detuvo. Puedes reintentar o volver al listado.
// //                             </Typography>
// //                           </Alert>
// //                         )}
// //                       </Box>
...
// //               {/* Acciones del stepper si hay error */}
// //               {procesSteps.some(s => s.status === "error") && (
// //                 <Box sx={{ mt: 2, display: "flex", gap: 2 }}>
// //                   <ButtonBase label="Reintentar"
// //                     startIcon={<CheckCircleOutline />}
// //                     onClick={handleConfirmarRecepcion}
// //                     sx={{ px: 3 }}
// //                   />
// //                   <ButtonBase label="Volver al Listado"
...
// //               La solicitud #{successData?.id} ha sido procesada completamente
// //             </Typography>

// //             <Box sx={{ display: "flex", gap: 2, justifyContent: "center", flexWrap: "wrap", mb: 4 }}>
// //               <Paper elevation={0} sx={{ px: 3, py: 2, borderRadius: 3, border: "1px solid #bbf7d0", bgcolor: "white", minWidth: 130 }}>
// //                 <Typography variant="h4" fontWeight={800} sx={{ color: "#15803d" }}>{successData?.totalItems}</Typography>
// //                 <Typography variant="caption" color="text.secondary" fontWeight={600}>Items procesados</Typography>
// //               </Paper>
// //               <Paper elevation={0} sx={{ px: 3, py: 2, borderRadius: 3, border: "1px solid #bbf7d0", bgcolor: "white", minWidth: 130 }}>
// //                 <Typography variant="h4" fontWeight={800} sx={{ color: "#4f46e5" }}>{successData?.kardexMovements}</Typography>
// //                 <Typography variant="caption" color="text.secondary" fontWeight={600}>Movimientos kardex</Typography>
// //               </Paper>
// //               <Paper elevation={0} sx={{ px: 3, py: 2, borderRadius: 3, border: "1px solid #bbf7d0", bgcolor: "white", minWidth: 130 }}>
// //                 <Typography variant="h4" fontWeight={800} sx={{ color: "#0369a1" }}>{successData?.equipmentUnits}</Typography>
// //                 <Typography variant="caption" color="text.secondary" fontWeight={600}>Equipos registrados</Typography>
// //               </Paper>
// //               <Paper elevation={0} sx={{ px: 3, py: 2, borderRadius: 3, border: "1px solid #bbf7d0", bgcolor: "white", minWidth: 130 }}>
// //                 <Typography variant="h4" fontWeight={800} sx={{ color: "#7c3aed" }}>ENTRADA</Typography>
// //                 <Typography variant="caption" color="text.secondary" fontWeight={600}>Tipo de movimiento</Typography>
// //               </Paper>
// //             </Box>
...
// //                       </Typography>
// //                     </Alert>
// //                     {!equiposCompletos && equiposItems.length > 0 && (
// //                       <Alert severity="warning" sx={{ mb: 2, borderRadius: 2 }}>
// //                         <Typography variant="body2" fontWeight={600}>
// //                           Faltan seriales por capturar. Todos los equipos deben estar pistoliados para confirmar.
// //                         </Typography>
// //                       </Alert>
// //                     )}
...
// //                   subtitle="Adjunta la guía de remisión. El sistema procesará el kardex y actualizará el inventario automáticamente."
// //                 />
...
// //                   <Paper variant="outlined" sx={{ flex: "1 1 140px", p: 2, borderRadius: 2, bgcolor: "#f0f9ff", border: "1px solid #bae6fd" }}>
...
// //                       {itemsRecepcion.filter(i => i.productType !== "EQUIPMENT").reduce((s, i) => s + (i.receivedQuantity ?? 0), 0) +
// //                        itemsRecepcion.filter(i => i.productType === "EQUIPMENT").reduce((s, i) => s + (i.serials?.length ?? 0), 0)}
// //                     </Typography>
// //                     <Typography variant="caption" color="text.secondary">movimientos ENTRY</Typography>
// //                   </Paper>
// //                   <Paper variant="outlined" sx={{ flex: "1 1 140px", p: 2, borderRadius: 2, bgcolor: "#f0fdf4", border: "1px solid #bbf7d0" }}>
// //                     <Stack direction="row" spacing={1} alignItems="center" mb={0.5}>
// //                       <StorageOutlined sx={{ fontSize: 16, color: "#15803d" }} />
// //                       <Typography variant="caption" fontWeight={700} color="#15803d">Inventario</Typography>
// //                     </Stack>
// //                     <Typography variant="h6" fontWeight={800} color="#15803d">{itemsRecepcion.length}</Typography>
// //                     <Typography variant="caption" color="text.secondary">líneas a actualizar</Typography>
// //                   </Paper>
// //                   <Paper variant="outlined" sx={{ flex: "1 1 140px", p: 2, borderRadius: 2, bgcolor: "#faf5ff", border: "1px solid #e9d5ff" }}>
// //                     <Stack direction="row" spacing={1} alignItems="center" mb={0.5}>
...
// //                       {itemsRecepcion.filter(i => i.productType === "EQUIPMENT").reduce((s, i) => s + (i.serials?.length ?? 0), 0)}
// //                     </Typography>
// //                     <Typography variant="caption" color="text.secondary">equipos a registrar</Typography>
// //                   </Paper>
// //                 </Box>
...
// //                   Se registrarán movimientos tipo <strong>ENTRY</strong> en el kardex y se actualizará el stock en hub_inventory. Esta acción es irreversible.
// //                 </Alert>
...
// //                     startIcon={<CheckCircleOutline />}
// //                     onClick={handleConfirmarRecepcion}
// //                     disabled={!archivoGuia || !codigoGuia || (equiposItems.length > 0 && !equiposCompletos)}
...
// //             </Box>
// //             <Box>
// //               <Typography variant="subtitle1" fontWeight={700} sx={{ color: "#1e293b" }}>Pistolear Equipo</Typography>
// //               {equipoSeleccionado && (
// //                 <Typography variant="caption" color="text.secondary">
...
// //                       {todosCompletos ? "⚡ Guardando automáticamente..." : `Completa todos los campos para auto-guardar el serial ${totalCap + 1}`}
// //                     </Typography>
// //                   </Box>
// //                 )}
...
// /**
//  * STEP 2 — Kardex batch
//  * EQUIPMENT → 1 movimiento por serial pistoliado (qty real = serials.length)
...
//           quantity: 1,
//           unitPrice: item.unitPrice ?? 0,
//           notes: `Recepción SR#${supplyRequestId} — ${serial.serialNumber}`,
//           referenceType: "SUPPLY_REQUEST",
//           referenceId: supplyRequestId,
//           serialNumber: serial.serialNumber,
//           createdBy,
