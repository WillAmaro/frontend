...
const validacionesCampo: Record<string, { regex: RegExp; mensaje: string }> = {
  serialNumber: {
    regex: /^[A-Z0-9]{14,20}$/,
    mensaje:
      "Serie inválida (alfanumérico, 14–20 chars, ej: ZTEATV45501950107)",
  },
  mac: {
...
const INITIAL_STEPS: ProcessStep[] = [
  {
    ...
    description:
      "Actualizando supply request con cantidades entregadas y guía de remisión...",
    status: "idle",
  },
  {
    label: "Kardex de Entrada",
    description:
      "Registrando movimientos ENTRY en el kardex (un registro por serial en equipos)...",
    status: "idle",
  },
  {
    label: "Inventario y Equipos",
    description:
      "Actualizando stock en hub_inventory y registrando equipment units con serial/MAC/UA...",
    status: "idle",
  },
];
...
export default function RecepcionMateriales() {
  ...
  const [catalogos, setCatalogos] = useState<DataCatalogoDTO>({
    companies: [],
    projects: [],
  });
  ...
  const [initialReceptionDate, setInitialReceptionDate] =
    useState<Dayjs | null>(dayjs().subtract(7, "day"));
  ...
  const [equipoSeleccionado, setEquipoSeleccionado] =
    useState<SupplyRequestItem | null>(null);
  ...
  const handleBuscarSolicitudes = async () => {
    ...
    try {
      const endDate =
        receptionDate?.format("YYYY-MM-DD") ?? dayjs().format("YYYY-MM-DD");
      const startDate =
        initialReceptionDate?.format("YYYY-MM-DD") ??
        dayjs().subtract(7, "day").format("YYYY-MM-DD");

      const params = new URLSearchParams({
        companyId: "1",
        projectId: String(selectedProject ?? 0),
        hubId: "1",
        startDate,
        endDate,
        ...(selectedStatus ? { status: selectedStatus } : {}), // ← solo si tiene valor
      });

      const res = await fetch(
        `${API_URL}/api/hub-supply/supply-requests?${params}`,
      );
  ...
  const handleVerDetalle = async (solicitudId: number) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/hub-supply/companies/${company}/supply-requests/${solicitudId}?projectId=${selectedProject}`,
      );
  ...
  const abrirModalEquipo = (item: SupplyRequestItem) => {
    ...
    setCamposError({});
    const existentes =
      itemsRecepcion.find((i) => i.itemCode === item.itemCode)?.serials ?? [];
  ...
  const checkAutoAgregar = (
    ...
    const todoCompleto = campos.every(
      (c) => !!(nuevoSerial as any)[c.field]?.trim(),
    );
    if (!todoCompleto) return;
    const hayErrores = campos.some(
      (c) => !!validarCampo(c.field, (nuevoSerial as any)[c.field] ?? ""),
    );
  ...
  const agregarSerial = () => {
    ...
    const tipo = getEquipoTipo(
      equipoSeleccionado.itemCode,
      equipoSeleccionado.itemDescription,
    );
    const campos = camposEquipo[tipo];
    ...
    campos.forEach((c) => {
      ...
      if (!val.trim()) {
        ...
        if (err) {
          nuevosErrores[c.field] = err;
          hayError = true;
        }
      }
    });
    if (hayError) {
      setCamposError(nuevosErrores);
      return;
    }
    setSerialésCapturados((prev) => [
      ...prev,
      { ...serialActual } as EquipoSerial,
    ]);
    setSerialActual({});
    setCamposError({});
  };
  ...
  const handleConfirmarRecepcion = async () => {
    if (!archivoGuia) {
      setErrorMsg("Adjunta la guía de remisión");
      return;
    }
    if (!codigoGuia) {
      setErrorMsg("Ingresa el código de guía");
      return;
    }
    setErrorMsg("");
    ...
    try {
      ...
      console.log(
        "[STEP 1] POST delivery payload:",
        JSON.stringify(deliveryPayload, null, 2),
      );

      const resDelivery = await fetch(`${API_URL}/api/hub-supply/delivery`, {
        method: "POST",
      ...
      if (!resDelivery.ok) {
        ...
        throw new Error(
          errBody.message ?? `HTTP ${resDelivery.status} al registrar delivery`,
        );
      }

      const deliveryResult = await resDelivery.json();
      const totalDelivered =
        deliveryResult.items?.length ?? itemsRecepcion.length;
      updateStep(0, {
        status: "success",
    ...
    try {
      ...
      console.log(
        "[STEP 2] POST kardex-batch payload:",
        JSON.stringify(kardexPayload, null, 2),
      );

      const resKardex = await fetch(
        `${API_URL}/api/hub-inventory/kardex-batch`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!resKardex.ok) {
        ...
        throw new Error(
          errBody.message ?? `HTTP ${resKardex.status} en kardex-batch`,
        );
      }

      const kardexResult = await resKardex.json();
      kardexMovementsCount =
        kardexResult.totalInserted ?? kardexPayload.movements.length;
      updateStep(1, {
        status: "success",
    ...
    try {
      const inventoryPayload = buildInventoryBatchPayload(
        itemsRecepcion,
        supplyRequestId,
        fecha,
      );
      ...
      console.log(
        "[STEP 3] POST inventory-batch payload:",
        JSON.stringify(inventoryPayload, null, 2),
      );

      const resInventory = await fetch(
        `${API_URL}/api/hub-inventory/inventory-batch`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!resInventory.ok) {
        ...
        throw new Error(
          errBody.message ?? `HTTP ${resInventory.status} en inventory-batch`,
        );
      }
  ...
  // ── Derivados ─────────────────────────────────────────────────────────
  const materialesItems = itemsRecepcion.filter(
    (i) => i.productType === "MATERIAL",
  );
  const herramientasItems = itemsRecepcion.filter(
    (i) => i.productType === "TOOL",
  );
  const equiposItems = itemsRecepcion.filter(
    (i) => i.productType === "EQUIPMENT",
  );
  ...
  const totalEquipmentUnitsPreview = itemsRecepcion
    .filter((i) => i.productType === "EQUIPMENT")
    .reduce((s, i) => s + (i.serials?.length ?? 0), 0);
    ...
    },
    {
      ...
      renderCell: (p) => (
        <Typography
          ...
          sx={{
            color:
              p.value === p.row.requestedQuantity
                ? "success.main"
                : "warning.main",
          }}
        >
  ...
  const columnasEquipos: GridColDef[] = [
    {
      ...
      renderCell: (p) => (
        <Chip
          label={p.value}
          size="small"
          sx={{ bgcolor: "#2e7d32", color: "white", fontWeight: 700 }}
        />
      ),
    },
    {
...

    {
      ...
      renderCell: (p) => (
        <Typography
          ...
          sx={{
            color:
              p.value === p.row.deliveredQuantity
                ? "success.main"
                : "warning.main",
          }}
        >
    ...
    // {
    //   field: "serials",
    //   headerName: "Pistoliados",
    //   width: 130,
    //   align: "center",
    //   headerAlign: "center",
    //   renderCell: (p) => {
    //     const cap =
    //       itemsRecepcion.find((i) => i.itemCode === p.row.itemCode)?.serials
    //         ?.length ?? p.row.deliveredQuantity;
    //     return (
    //       <Chip
    //         label={`${cap} / ${p.row.requestedQuantity}`}
    //         size="small"
    //         color={
    //           cap === p.row.requestedQuantity
    //             ? "success"
    //             : cap > 0
    //               ? "warning"
    //               : "default"
    ...
    // },
    {
      ...
      headerAlign: "center",
      renderCell: (p) => {
        // Pistoleados en sesión actual (estado local)
        const serialesLocales =
          itemsRecepcion.find((i) => i.itemCode === p.row.itemCode)?.serials
            ?.length ?? 0;

        // Pistoleados previos guardados en BD (deliveredQuantity)
        const previosBD = p.row.deliveredQuantity ?? 0;

        // Si hay actividad local en esta sesión → usa locales
        // Si no → usa lo que ya estaba guardado en BD
        const pistoliados = serialesLocales > 0 ? serialesLocales : previosBD;

        const total = p.row.requestedQuantity;
        const completo = pistoliados === total;
        const parcial = pistoliados > 0 && pistoliados < total;

        return (
          <Chip
            label={`${pistoliados} / ${total}`}
            size="small"
            color={completo ? "success" : parcial ? "warning" : "default"}
            sx={{ fontWeight: 700 }}
          />
        );
      },
    },
    {
      ...
      sortable: false,
      renderCell: (p) => {
        const isDisabled = solicitudDetalle.status === "DELIVERED"
  ...
  return (
    ...
    >
                        ...
                        >
                          ...
                          {step.status === "idle" && (
                            <Typography
                              ...
                              sx={{
                                color:
                                  index <= activeStep ? "#4f46e5" : "#94a3b8",
                              }}
                            >
                              {index + 1}
                            </Typography>
                        ...
                        {step.status === "error" && (
                          ...
                          >
                            ...
                            <Typography
                              ...
                              mt={0.5}
                            >
                              El proceso se detuvo en este paso. Puedes
          ...
          >
            ...
            <Box
              sx={{
                ...
                justifyContent: "center",
                boxShadow:
                  "0 0 0 16px rgba(34,197,94,0.12), 0 0 0 32px rgba(34,197,94,0.06)",
                ...
                "@keyframes pulseRing": {
                  ...
                  "50%": {
                    boxShadow:
                      "0 0 0 20px rgba(34,197,94,0.08), 0 0 0 40px rgba(34,197,94,0.03)",
                  },
                },
              }}
            >
            ...
            >
              La solicitud #{successData?.id} fue procesada en 3 pasos
              correctamente
            </Typography>
            ...
            >
              {[
                {
                  value: successData?.totalItems,
                  label: "Items procesados",
                  color: "#15803d",
                },
                {
                  value: successData?.kardexMovements,
                  label: "Movimientos kardex",
                  color: "#4f46e5",
                },
                {
                  value: successData?.equipmentUnits,
      ...
      {/* ══════════════ VISTA LISTADO ══════════════════════════════════════ */}
      <Fade
        in={vista === "list" && !showSuccess && !showStepper}
        timeout={500}
        unmountOnExit
      >
            ...
            >
              ...
              >
                <Stack spacing={2.5}>
                  ...
                  <Typography
                    ...
                    sx={{ lineHeight: 1.6 }}
                  >
                    Filtra solicitudes aprobadas disponibles para recibir en la
                    fecha seleccionada.
                  </Typography>
                  <DatePickerBase
                    value={initialReceptionDate}
                    setValue={setInitialReceptionDate}
              ...
              >
                ...
                >
                  ...
                  >
                    <FilterListOutlined sx={{ fontSize: 14 }} /> Solo
                    solicitudes con estado APPROVED
                  </Typography>
                  <ButtonBase
              ...
              >
                <Stack spacing={2}>
                  ...
                  {/* ── Proyecto y Estado en la misma fila ── */}
                ...
                >
                  ...
                  >
                    <FilterListOutlined sx={{ fontSize: 14 }} />
                    {selectedStatus
                      ? `Filtrando por: ${selectedStatus}`
                      : "Todos los estados"}
                  </Typography>
                  <ButtonBase
                    onClick={handleBuscarSolicitudes}
            ...
            <Fade in timeout={400}>
              ...
              >
                ...
                <CustomDataGrid
                  ...
                  onSearch={setSearchListado}
                  pageSize={10}
                  onView={(row) => handleVerDetalle(row.id)}
                />
              </Card>
            </Fade>
          )}
          ...
          {!loadingList && solicitudes.length === 0 && (
            ...
            >
              ...
              <Typography variant="body2" color="text.disabled">
                Selecciona fecha y proyecto, luego haz clic en "Buscar
                Solicitudes".
              </Typography>
            </Card>
          )}
      ...
      {/* ══════════════ VISTA DETALLE ══════════════════════════════════════ */}
      <Fade
        in={vista === "detail" && !showSuccess && !showStepper}
        timeout={600}
        unmountOnExit
      >
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <>
              ...
              >
                ...
                >
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        Verifica cantidades. Los equipos deben pistoliarse
                        individualmente.
                      </Typography>
                    </Box>
                  </Stack>
                  <Stack direction="row" spacing={1.5}>
                    <Chip
                      label={solicitudDetalle.status}
                      color="success"
                      sx={{ fontWeight: 800 }}
                    />
                    <Chip
                      label={`${solicitudDetalle.items?.length ?? 0} items`}
                      variant="outlined"
                      sx={{ fontWeight: 700 }}
                    />
                  </Stack>
                ...
                <Box sx={{ display: "flex", gap: 2, mt: 3, flexWrap: "wrap" }}>
                  {(["MATERIAL", "TOOL", "EQUIPMENT"] as const).map((tipo) => {
                    const items = itemsRecepcion.filter(
                      (i) => i.productType === tipo,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <Typography
                          variant="h4"
                          fontWeight={800}
                          sx={{ color: cfg.color, lineHeight: 1.2, mt: 0.5 }}
                        >
                          {items.length}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
              ...
              >
                <Box sx={{ borderBottom: 1, borderColor: "divider", mb: 3 }}>
                  <Tabs value={tabActual} onChange={(_, v) => setTabActual(v)}>
                    <Tab
                      label={`📦 Materiales (${materialesItems.length})`}
                      sx={{ fontWeight: 700, textTransform: "none" }}
                    />
                    <Tab
                      label={`🔧 Herramientas (${herramientasItems.length})`}
                      sx={{ fontWeight: 700, textTransform: "none" }}
                    />
                    <Tab
                      label={`⚙️ Equipos (${equiposItems.length})`}
                      sx={{ fontWeight: 700, textTransform: "none" }}
                    />
                  </Tabs>
                </Box>
                ...
                {tabActual === 0 && (
                  <Box>
                    ...
                    />
                    <CustomDataGrid
                      columns={buildColumnas(categoriaConfig.MATERIAL.color)}
                      localRows={filtrar(materialesItems, searchMat).map(
                        (i) => ({ ...i, id: i.itemCode }),
                      )}
                      serverSide={false}
                      search=""
                      ...
                      pageSize={30}
                      sx={{
                        border: `2px solid ${categoriaConfig.MATERIAL.color}20`,
                      }}
                    />
                  </Box>
                )}
                ...
                {tabActual === 1 && (
                  <Box>
                    ...
                    />
                    <CustomDataGrid
                      columns={buildColumnas(categoriaConfig.TOOL.color)}
                      localRows={filtrar(herramientasItems, searchTool).map(
                        (i) => ({ ...i, id: i.itemCode }),
                      )}
                      serverSide={false}
                      search=""
                      ...
                      pageSize={30}
                      sx={{
                        border: `2px solid ${categoriaConfig.TOOL.color}20`,
                      }}
                    />
                  </Box>
                )}
                ...
                {tabActual === 2 && (
                  <Box>
                    <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                      <Typography variant="body2" fontWeight={600}>
                        Cada equipo debe identificarse individualmente. Usa{" "}
                        <strong>"Pistolear"</strong> para capturar serie, MAC,
                        UA, MTA MAC según el tipo.
                      </Typography>
                    </Alert>
                    {equiposItems.some(
                      (eq) => (eq.serials?.length ?? 0) < eq.requestedQuantity,
                      ...
                      equiposItems.length > 0 && (
                        <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                          <Typography variant="body2" fontWeight={600}>
                            Puedes confirmar con entrega parcial. Solo se
                            registrarán los equipos ya pistoliados. Los
                            pendientes quedan en la solicitud para una próxima
                            recepción.
                          </Typography>
                        </Alert>
                      )}
                    ...
                    <CustomDataGrid
                      ...
                      localRows={filtrar(equiposItems, searchEq).map((i) => ({
                        ...i,
                        id: i.itemCode,
                      }))}
                      serverSide={false}
                      ...
                      pageSize={30}
                      sx={{
                        border: `2px solid ${categoriaConfig.EQUIPMENT.color}20`,
                      }}
                    />
                  </Box>
                )}
              ...
              >
                ...
                <Box sx={{ display: "flex", gap: 2, mb: 3, flexWrap: "wrap" }}>
                  ...
                  >
                    ...
                    <Typography variant="h6" fontWeight={800} color="#92400e">
                      {itemsConEntrega.length}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      ítems a marcar (
                      {itemsRecepcion.length - itemsConEntrega.length} sin
                      entrega)
                    </Typography>
                  </Paper>
                  <Paper
                    ...
                    <Fade in timeout={400}>
                      ...
                      >
                        {archivoGuia.type === "application/pdf" ? (
                          ...
                          <ImageOutlined
                            sx={{ color: "info.main", fontSize: 32 }}
                          />
                        )}
                        <Box sx={{ flex: 1 }}>
                          <Typography variant="body2" fontWeight={600}>
                ...
                <Alert severity="warning" sx={{ borderRadius: 2, mb: 3 }}>
                  Se registrarán movimientos <strong>ENTRY</strong> en el kardex
                  y se actualizará el stock. Los equipos serán registrados con
                  serial, MAC y UA capturados.{" "}
          ...
          {equipoSeleccionado &&
            (() => {
              ...
              const todosCompletos = campos.every(
                (c) => !!(serialActual as any)[c.field]?.trim(),
              );
                ...
                <>
                  ...
                  <Box
                    sx={{
                      ...
                      p: 2,
                      borderRadius: 2,
                      bgcolor: autoGuardar ? "#f0fdf4" : "#f8fafc",
                      border: `1px solid ${autoGuardar ? "#bbf7d0" : "#e2e8f0"}`,
                      transition: "all 0.3s",
                    }}
                  >
                    <FormControlLabel
                      ...
                      label={
                        ...
                        >
                          ...
                          <Typography
                            ...
                            fontWeight={700}
                            sx={{
                              color: autoGuardar ? "#15803d" : "text.secondary",
                            }}
                          >
                  ...
                  <Stack spacing={2} sx={{ mb: 3 }}>
                    {campos.map((campo) => {
                      ...
                      const esValido = regla
                        ? regla.regex.test(valor.trim()) && valor.trim() !== ""
                        : valor.trim() !== "";
                          ...
                          <TextField
                            ...
                            sx={{
                              "& .MuiOutlinedInput-root": {
                                bgcolor:
                                  totalCap >= totalReq ? "#f8fafc" : "white",
                                transition: "all 0.2s",
                                ...(esValido && {
                                  "& fieldset": {
                  ...
                  {!autoGuardar && (
                    <ButtonBase
                      ...
                      label={
                        totalCap >= totalReq
                          ? "✓ Cantidad completa"
                          : `Agregar Serial ${totalCap + 1} de ${totalReq}`
                      }
                      startIcon={<CheckCircle />}
                      onClick={agregarSerial}
                      disabled={
                        !serialActual.serialNumber?.trim() ||
                        totalCap >= totalReq
                      }
                      sx={{ mb: 3 }}
                    />
                  ...
                  {autoGuardar && totalCap < totalReq && (
                    <Box
                      sx={{
                        ...
                        border: `1px solid ${todosCompletos ? "#86efac" : "#e2e8f0"}`,
                        textAlign: "center",
                        transition: "all 0.3s",
                      }}
                    >
                      <Typography
                        ...
                        fontWeight={600}
                        sx={{
                          color: todosCompletos ? "#15803d" : "text.disabled",
                        }}
                      >
                        {todosCompletos
